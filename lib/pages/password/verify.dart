...
import 'package:online_music_player/pages/password/passchange.dart';
...
class OtpVerify extends StatefulWidget {
  ...
  @override
  State<OtpVerify> createState() => _OtpVerifyState(user);
}

class _OtpVerifyState extends State<OtpVerify>{
  User user;
  _OtpVerifyState(this.user);
      ...
      if (await otp.verifyOTP(otp: x)) {
        ...
        Timer(const Duration(seconds: 1), () {
          ...
          Navigator.of(context).pushReplacement(
              MaterialPageRoute(builder: (context) => PassChange(user)));
        });
      } else {
  ...
  Widget build(BuildContext context) {
    final defaultPinTheme = PinTheme(
      ...
      decoration: BoxDecoration(
        border: Border.all(color: Color.fromARGB(255, 3, 203, 203)),
